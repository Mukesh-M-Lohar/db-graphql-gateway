...
!!! warning "Algorithm Confusion Attacks"
...
!!! success "Database-Level Security"
...
GraphQL's deeply nested architecture is notoriously vulnerable to Denial of Service (DoS) via exponentially recursive or highly aliased queries. The Gateway mitigates this via Strawberry **AST Validation Rules** executed before the query ever hits the resolver logic.

<div class="grid cards" markdown>

-   :material-arrow-down-bold: **Max Depth Rule**
    ---
    Rejects queries exceeding a configured depth limit, preventing infinite recursive relationship lookups.

-   :material-content-copy: **Max Aliases Rule**
    ---
    Prevents expansive execution fan-out attacks by capping the number of field aliases allowed in a single request.

-   :material-calculator: **Max Complexity Rule**
    ---
    Computes a total point budget for the query based on field selection and nesting, rejecting queries that exceed the budget.
